...
## KJL: install/update BiocManager (using v3.14 since this allows updates to a lot of packages)
...
## KJL: install/update all packages loaded below (force re-install for Bioconductor packages)
...
## KJL: make sure R is v4.1.1 at least and GenomeInfoDb is and v1.30.0 at least
sessionInfo()
## 
...
## KJL: creating directory reference for files 
...
## KJL: changing annotations to hg38 (v75 is for hg19) 
...
ExpressionPlot(
  object = husc500Ag,
  features = c("PAX7", "PAX3", "MYC", "MYF5", "MYOG", "MYOD1"),
  group.by = "sample_id",
  assay = "RNA"
)
...
ExpressionPlot(
  object = husc500Ag,
  features = c("CDKN1B","CDKN1C", "CDKN2A", "YAP1", "MKI67"),
  group.by = "sample_id",
  assay = "RNA"
)
...
VlnPlot(husc500Ag, features = c("IL1RL1", "CCL7", "CCL8"),
        pt.size = 0.2, ncol = 3)

ExpressionPlot(
  object = husc500Ag,
  features = c("CCL20", "CCL2", "CCL7", "CCL8", "CXCL14", "TNFRSF12A", "IL33", "CXCL8", "IL1RL1", "CD9", "CD44", "CD34"),
  group.by = "sample_id",
  assay = "RNA"
)
...
## adjusted p-values and set below 0.05 as a significant threshold 
...
length(open_1) ## 16 are significantly up
open_2 <- rownames(da_peaks[da_peaks$p_val_adj<0.05 & da_peaks$avg_log2FC<0, ])
length(open_2) ## 31 are significantly down
...
sig_gene_filt <- read.csv("v4_sig_da_peaks_filtered_3col.csv", check.names = FALSE)
sig_gene_filt

sig_gene_mat <- data.matrix(sig_gene_filt)

heatmap(sig_gene_mat, main = "Significant peaks in Activated and Quiescent Hu-MuSC", trace= "none", margins = c(10,12))

#Extract just the numeric data into a matrix with named rows by gene
rownames(geneExp) <- geneExp$p_val_adj
sig_genes_table <- data.matrix(geneExp[2:47]) #adjust the number [2:47] depending on the rows and columns you want
head(geneExp_matrix)
...
CoveragePlot(
object = husc500Ag,
region = "chr22-48536323-48537872",
group.by = "sample_id",
...
## heatmap
dev.off() 
plot_gsva_heatmap(gsva_result, max_pathways = 30, group.by = "sample_id")
...
plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-354194")
...
plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-166020")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-451307")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-1299503")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-190374")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-380108")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-5676594")

plot_gsva_pathway(gsva_result, pathway_id = "R-HSA-391908")


#graphs for relevant pathways
dev.off()
relevant_pathways <- c("R-HSA-1059683", "R-HSA-9014843",  "R-HSA-109703", "R-HSA-109704", "R-HSA-110056","R-HSA-1299361", "R-HSA-112122", "R-HSA-141334", "R-HSA-1483196", "R-HSA-166020")
...
relevant_pathways <- c("R-HSA-110056", "R-HSA-6783589", "R-HSA-196299", "R-HSA-451927", "R-HSA-9008059", "R-HSA-391908", "R-HSA-916853", "R-HSA-1296025", "R-HSA-166020", "R-HSA-1855231")
plot_gsva_heatmap(gsva_result, 
                  pathway_ids = relevant_pathways, # limit to these pathways
                  margins = c(6,20), # adapt the figure margins in heatmap.2
                  dendrogram = "col", # only plot column dendrogram
...
relevant_pathways <- c("R-HSA-6783589", "R-HSA-196299", "R-HSA-512988", "R-HSA-1059683", "R-HSA-8983432", "R-HSA-525793", "R-HSA-451927", "R-HSA-9008059")
plot_gsva_heatmap(gsva_result, 
                  pathway_ids = relevant_pathways, # limit to these pathways
                  margins = c(6,20), # adapt the figure margins in heatmap.2
                  dendrogram = "col", # only plot column dendrogram
